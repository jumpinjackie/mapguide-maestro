...
namespace OSGeo.MapGuide.MaestroAPI.PackageBuilder
{
    public partial class PackageProgress : Form
    {
        ...
        /// </summary>
        private readonly static MaestroAPI.PackageBuilder.ProgressType[] BUILD_STAGES = null;

        static PackageProgress()
        {
            BUILD_STAGES = 
            new OSGeo.MapGuide.MaestroAPI.PackageBuilder.ProgressType[] {
                MaestroAPI.PackageBuilder.ProgressType.ReadingFileList,
                MaestroAPI.PackageBuilder.ProgressType.PreparingFolder,
                MaestroAPI.PackageBuilder.ProgressType.MovingResources,
                MaestroAPI.PackageBuilder.ProgressType.Compressing
            };
        }
        ...
        public static Dictionary<string, MaestroAPI.PackageBuilder.ResourceItem> ListPackageContents(Form owner, MaestroAPI.ServerConnectionI connection, string packageFile)
        {
            ...
            if (pkgp.ShowDialog(owner) == DialogResult.OK)
                return (Dictionary<string, MaestroAPI.PackageBuilder.ResourceItem>)pkgp.m_invokeResult;
            else
                return null;
        }
        ...
        public static DialogResult UploadPackage(Form owner, MaestroAPI.ServerConnectionI connection, string packageFile)
        {
            PackageProgress pkgp = new PackageProgress();
            pkgp.m_invokeArgs = new object[] { packageFile };
            pkgp.m_invokeObj = new PackageBuilder(connection);
        ...
        /// <returns></returns>
        public static DialogResult CreatePackage(Form owner, MaestroAPI.ServerConnectionI connection, string folderResourceId, string zipfilename, List<string> allowedExtensions, bool removeExistingFiles, string alternateTargetResourceId)
        {
            PackageProgress pkgp = new PackageProgress();
        ...
        private void PackageProgress_Load(object sender, EventArgs e)
        {
            ...
            m_invokeObj.Progress += new OSGeo.MapGuide.MaestroAPI.PackageBuilder.ProgressDelegate(SetCurrentProgress);
        }
        ...
        private delegate void SetCurrentProgressDelegate(MaestroAPI.PackageBuilder.ProgressType type, string resource, int total, int pg);
        private void SetCurrentProgress(MaestroAPI.PackageBuilder.ProgressType type, string resource, int total, int pg)
        {
            ...
            else
            {
                ...
                if (type == OSGeo.MapGuide.MaestroAPI.PackageBuilder.ProgressType.Uploading)
                {
                    HideTotal();
                    if (pg == total)
                    {
