...
{
    internal class VectorLayer400ItemTemplate : ItemTemplate
    {
        public VectorLayer400ItemTemplate()
        {
            Category = Strings.TPL_CATEGORY_MGOS40;
            Icon = Res.layer;
            Description = Strings.TPL_VLDF_400_DESC;
            Name = Strings.TPL_VLDF_400_NAME;
        ...
        public override IResource CreateItem(string startPoint, OSGeo.MapGuide.MaestroAPI.IServerConnection conn)
        {
            using (var picker = new ResourcePicker(conn, ResourceTypes.FeatureSource.ToString(), ResourcePickerMode.OpenResource))
            {
                ...
                {
                    var lyr = ObjectFactory.CreateDefaultLayer(OSGeo.MapGuide.ObjectModels.LayerDefinition.LayerType.Vector, new Version(4, 0, 0));
