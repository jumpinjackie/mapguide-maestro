...
namespace Maestro.MapPublisher.Common
{
    public class StaticMapPublisher : IProgress<TileProgress>
    {
        ...
        {
            return $"{options.MapAgent.Endpoint}?USERNAME={options.MapAgent.Username}&PASSWORD={options.MapAgent.Password}&LOCALE=en&CLIENTAGENT=Maestro.MapPublisher&OPERATION=GETTILEIMAGE&VERSION=1.2.0&MAPDEFINITION={getResource(options)}&BASEMAPLAYERGROUPNAME={getGroupName(options)}&SCALEINDEX={{z}}&TILEROW={{x}}&TILECOL={{y}}";
        }
